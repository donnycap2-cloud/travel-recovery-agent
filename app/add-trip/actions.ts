...
export type FetchFlightDataState =
  | {
      ...
      message?: string;
      fieldErrors?: Partial<
        Record<"flight1" | "flight2" | "date" | "originAirport", string>
      >;
      values?: {
        flight1?: string;
        flight2?: string;
  ...
  | {
      ...
      data: {
        flight1: Required<
          Pick<
            FlightLeg,
            "flightNumber" | "origin" | "destination" | "arrivalTime" | "departureTime"
          >
        >;
        flight2: Required<
          Pick<
...
export async function fetchFlightData(
  ...
  else if (!isValidFlightNumber(flight1))
    fieldErrors.flight1 = "Use format like AA123.";

  if (!flight2) fieldErrors.flight2 = "Enter flight 2 number.";
  else if (!isValidFlightNumber(flight2))
    fieldErrors.flight2 = "Use format like AA123.";
  ...
  const arrivalMs = parseLocalTime(flight1Resolved.scheduledArrival);
  const departMs = parseLocalTime(flight2Resolved.scheduledDeparture);

  if (arrivalMs === null || departMs === null) {
    return {
      status: "error",
      message: "Invalid flight timing data.",
      values: {
        flight1: rawFlight1,
        flight2: rawFlight2,
        date: rawDate
